class Level {
    ...
    loadWorld(): any {
        ...
        function AddWall(x, y, big: boolean) {
            if (big) { x += 0.5; y += 0.5 }
            var size = big ? 1 : 0.5;
            var wall = world.createBody(Vec2(x, y));
            wall.createFixture(pl.Box(size, size), wallFD);
        }
        function AddTriangle(x, y, rotation: number, big: boolean) {
            if (big) { x += 0.5; y += 0.5 }
            var size = big ? 1 : 0.5;
            // if (big) {
            //     if (rotation == 0) {
        ...
        function AddCurve(x, y, rotation: number) {
        ...
        function AddGoal(x, y, big: boolean) {
            if (big) { x += 0.5; y += 0.5 }
            var size = big ? 1 : 0.5;
        ...
        function AddPusher(x, y, direction: Direction) {
        ...
        for (var lineNum = 0; lineNum < this.levelString.split('\n').length; lineNum++) {
            ...
            for (var charNum = 0; charNum < line.length; charNum++) {
                ...
                if (cha == '#') AddWall(x, y, true);
                if (cha == 'o') AddBouncer(x + 0.5, y + 0.5, 1);
                if (cha == '.') AddPin(x + 0.5, y + 0.5);
                if (cha == ':') { AddPin(x + 0.5, y); AddPin(x + 0.5, y + 1); }
                if (cha == '…') { AddPin(x, y + 0.5); AddPin(x + 1, y + 0.5); }
                if (cha == '+') { AddPin(x, y + 0.5); AddPin(x + 1, y + 0.5); AddPin(x + 0.5, y + 1); AddPin(x + 0.5, y); }
                if (cha == 'x') this.ball = AddBall(x + 0.5, y + 0.5);

                if (cha == '◢') AddTriangle(x, y, Math.PI / 2, true);
                if (cha == '◣') AddTriangle(x, y, 0, true);
                if (cha == '◤') AddTriangle(x, y, -Math.PI / 2, true);
                if (cha == '◥') AddTriangle(x, y, Math.PI, true);

                if (cha == 'g') AddGoal(x, y, true);

                if (cha == '^') this.pushers.push(AddPusher(x, y, Direction.Up));
                if (cha == 'v') this.pushers.push(AddPusher(x, y, Direction.Down));
                if (cha == '>') this.pushers.push(AddPusher(x, y, Direction.Right));
                if (cha == '<') this.pushers.push(AddPusher(x, y, Direction.Left));

                if (cha == 'm') { AddBreakWall(x, y); AddBreakWall(x + 1, y); }

                if (cha == '◞') AddCurve(x, y, Math.PI / 2);
                if (cha == '◟') AddCurve(x, y, 0);
                if (cha == '◜') AddCurve(x, y, -Math.PI / 2);
                if (cha == '◝') AddCurve(x, y, Math.PI);
            }
        }
